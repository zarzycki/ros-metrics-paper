...
%https://www.sciencedirect.com/science/article/pii/S0022169417301452
...
\caption{Statistics of flagged events over the SRB in each data product using FIXED thresholds (top) and RELATIVE thresholds (bottom). ROF$_t$ and dSWE$_t$ represent the daily runoff and snow loss thresholds for that line. Events represents the number of events flagged over the 1980-2005 period. Duration is the average number of consecutive days an event lasts for. dSWE, ROF, and Precip represent the average snow loss, runoff, and precipitation per event by calculating the mean values across an event then averaging those. The same variables with a subscript `max'  find compute the single-day maximum during a given event and then average the maxima.}
\label{table:means}
\end{table}
