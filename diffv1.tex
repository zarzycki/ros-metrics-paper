...
%DIF DEL main_v1.tex   Thu Jun  6 09:19:53 2024
%DIF ADD main.tex      Tue Jul 16 15:09:29 2024
...
While studying historical events is important for planning purposes, and \DIFdelbegin \DIFdel{historical }\DIFdelend \DIFaddbegin \DIFadd{counterfactual }\DIFaddend reforecasts using imposed warming approaches can provide clues as to how similar events may unfold in the future (e.g., \citet{pettett2023the}), it is difficult for such approaches to provide risk quantification from a frequency-of-occurrence perspective. For these climatological evaluations, it is desirable to be able to identify such events in both observational datasets and model simulations, including future climate projections.
...
\subsection{Datasets}
\DIFaddbegin \label{subsec:datasets}
\DIFaddend 
...
\DIFaddbegin \label{subsec:algo}
\DIFaddend 

\DIFaddbegin \DIFadd{A schematic of the RoS detection algorithm described here is shown in Fig. \ref{fig:schematic}.
}\DIFaddend To identify RoS events \DIFaddbegin \DIFadd{in the datasets}\DIFaddend , three hydrometeorological variables are \DIFdelbegin \DIFdel{extracted over the SRB.
These include }\DIFdelend \DIFaddbegin \DIFadd{used; }\DIFaddend precipitation (PRECIP\DIFaddbegin \DIFadd{, includes both rainfall and snowfall), surface water runoff (ROF}\DIFaddend ), \DIFaddbegin \DIFadd{and }\DIFaddend snow water equivalent (SWE)\DIFdelbegin \DIFdel{, and surface water runoff (ROF)}\DIFdelend .
...
Grid cells that have at least 50\% of their area enclosed by the \DIFdelbegin \DIFdel{boundary of the SRB }\DIFdelend \DIFaddbegin \DIFadd{shapefile boundary of a river basin (here, the SRB) }\DIFaddend are kept, while those exterior to the \DIFdelbegin \DIFdel{SRB }\DIFdelend \DIFaddbegin \DIFadd{basin }\DIFaddend are set to missing values \DIFdelbegin \DIFdel{.
The resulting }\DIFdelend \DIFaddbegin \DIFadd{(Fig. \ref{fig:schematic}a).
The resulting SRB }\DIFaddend domains for each product can be seen in Fig. \ref{fig:means}.
...
This results in a one-dimensional time series with a single value for each calendar day that represents the \DIFdelbegin \DIFdel{SRB }\DIFdelend basin-averaged conditions for each data product.
RoS days are \DIFaddbegin \DIFadd{then }\DIFaddend defined by flagging periods of \DIFdelbegin \DIFdel{negative dSWE and positive ROF }\DIFdelend \DIFaddbegin \DIFadd{positive ROF and negative dSWE (snow loss) }\DIFaddend that both exceed specified thresholds ($t$)\DIFdelbegin \DIFdel{on the same days.
...
}\DIFaddend We test two methods of thresholding; one uses fixed thresholds across all four datasets (FIXED) and the other defines dataset-specific thresholds by those exceeding 95\% of all daily values (RELATIVE).
For FIXED, we require an average \DIFdelbegin \DIFdel{dSWE of -1.4 }\DIFdelend \DIFaddbegin \DIFadd{ROF of 1.4 }\DIFaddend mm/day (\DIFdelbegin \DIFdel{$t_{\textrm{dSWE}}$) and ROF of 1.4 }\DIFdelend \DIFaddbegin \DIFadd{$t_{\textrm{ROF}}$) and dSWE of -1.4 }\DIFaddend mm/day (\DIFdelbegin \DIFdel{$t_{\textrm{ROF}}$}\DIFdelend \DIFaddbegin \DIFadd{$t_{\textrm{dSWE}}$}\DIFaddend ) averaged across the \DIFdelbegin \DIFdel{SRB }\DIFdelend \DIFaddbegin \DIFadd{basin }\DIFaddend based on a manual sensitivity analysis.
Thresholds in the RELATIVE configuration are calculated independently for each \DIFdelbegin \DIFdel{dataset by setting the threshold to the }\DIFdelend \DIFaddbegin \DIFadd{product by computing the }\DIFaddend 95\% \DIFdelbegin \DIFdel{value of its own }\DIFdelend \DIFaddbegin \DIFadd{percentile value for each variable in the unsmoothed daily }\DIFaddend 1985-2005 time series \DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{(Fig. \ref{fig:schematic}b).
...
\DIFaddend To enforce a criterion that precipitation occurs during at least some portion of the event, we require an average PRECIP of 2 mm/day ($t_{\textrm{PRECIP}}$), which is kept the same in both the FIXED and RELATIVE configurations.
...
\DIFaddbegin \DIFadd{A sample RoS event detection is shown in Fig. \ref{fig:schematic}c.
...
Others include some measure of snowmelt, such as \citet{freudiger2014large} and \citet{li2019the} who required 3 mm of rainfall, 10 mm SWE, and \DIFdelbegin \DIFdel{a 20\% reduction in the snowpack (implied dSWE $<=$ -20 mm/day}\DIFdelend \DIFaddbegin \DIFadd{no larger than a 4:1 ratio between rainfall and snowmelt in order to eliminate rain-only events (\mbox{%DIFAUXCMD
\citet{musselman2018projected} }\hskip0pt%DIFAUXCMD
also includes the latter requirement}\DIFaddend ).
...
\DIFaddbegin \label{subsec:climatology}
\DIFaddend 
...
\DIFaddbegin \label{subsec:flaggedevents}
\DIFaddend 
...
The number of events in L15 increases because the ROF threshold \DIFaddbegin \DIFadd{$t_{\textrm{ROF}}$ }\DIFaddend (95th percentile of daily values) is reduced from 1.4 to 0.6 mm/day.
Similarly, the dSWE threshold \DIFaddbegin \DIFadd{$t_{\textrm{dSWE}}$ }\DIFaddend magnitude is reduced from -1.4 mm/day to -0.8 mm/day in NLDAS.
...
\DIFaddbegin \DIFadd{We also perform a sensitivity analysis intended to include the requirement in \mbox{%DIFAUXCMD
\citet{freudiger2014large} }\hskip0pt%DIFAUXCMD
and \mbox{%DIFAUXCMD
\citet{musselman2018projected} }\hskip0pt%DIFAUXCMD
that the sum of rainfall and snowmelt contains at least 20\% snowmelt.
...
We refer to this simulation as RELATIVE\_F14 since it preserves the same thresholds in RELATIVE with this one additional exclusionary check from \mbox{%DIFAUXCMD
\citet{freudiger2014large} }\hskip0pt%DIFAUXCMD
in order to remove high rainfall (but low snow loss) events.
...
\begin{table}
\caption{Statistics of RoS events over the SRB using FIXED thresholds (top) and RELATIVE thresholds (bottom). \DIFdelbeginFL \DIFdelFL{$t_{\textrm{dSWE}}$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$t_{\textrm{PRECIP}}$}\DIFaddendFL , $t_{\textrm{ROF}}$, and \DIFdelbeginFL \DIFdelFL{$t_{\textrm{PRECIP}}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$t_{\textrm{dSWE}}$ }\DIFaddendFL represent the thresholds used for \DIFdelbeginFL \DIFdelFL{snow water equivalent loss}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{precipitation}\DIFaddendFL , surface runoff, and \DIFdelbeginFL \DIFdelFL{precipitation}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{snow water equivalent loss}\DIFaddendFL , respectively (mm/day). Events represent the number of RoS events flagged over the 1980-2005 period. Duration is the average number of consecutive days an RoS event lasts. \DIFdelbeginFL \DIFdelFL{dSWE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{PRECIP}\DIFaddendFL $_{a}$, ROF$_{a}$, and \DIFdelbeginFL \DIFdelFL{PRECIP}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{dSWE}\DIFaddendFL $_{a}$ represent the \DIFdelbeginFL \DIFdelFL{average snow loss, }\DIFdelendFL amount of \DIFdelbeginFL \DIFdelFL{runoff }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{precipitation }\DIFaddendFL rate, \DIFdelbeginFL \DIFdelFL{and }\DIFdelendFL amount of \DIFdelbeginFL \DIFdelFL{precipitation }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{runoff }\DIFaddendFL rate\DIFaddbeginFL \DIFaddFL{, and average snow loss }\DIFaddendFL (mm/day) per event by calculating the mean daily value for each individual event and then averaging those.}
\DIFdelbeginFL %DIFDELCMD < \begin{tabular}{lcccccccc}
%DIFDELCMD <       %%%
\DIFdelendFL \DIFaddbeginFL \begin{tabular}{lccccccccc}
      \DIFaddendFL &  \DIFdelbeginFL \DIFdelFL{$t_{\textrm{dSWE}}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$t_{\textrm{PRECIP}}$ }\DIFaddendFL & $t_{\textrm{ROF}}$ & \DIFdelbeginFL \DIFdelFL{$t_{\textrm{PRECIP}}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$t_{\textrm{dSWE}}$ }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{$t_{\textrm{fSWE}}$ }& \DIFaddendFL Events & Duration & \DIFdelbeginFL \DIFdelFL{dSWE}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{PRECIP}\DIFaddendFL $_{a}$ & ROF$_{a}$ & \DIFdelbeginFL \DIFdelFL{PRECIP}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{dSWE}\DIFaddendFL $_{a}$ \\
      &  \textit{mm day$^{-1}$} & \textit{mm day$^{-1}$} & \textit{mm day$^{-1}$} & \DIFaddbeginFL \textit{\DIFaddFL{\%}} & \DIFaddendFL \textit{\#} & \textit{days} & \textit{mm day$^{-1}$} & \textit{mm day$^{-1}$} & \textit{mm day$^{-1}$} \\ \hline
FIXED &         &         &         &        &     &              &              &                \DIFaddbeginFL &                \DIFaddendFL \\ \hline
L15 & \DIFdelbeginFL \DIFdelFL{-1.4     }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{2.0 }\DIFaddendFL & 1.4 & \DIFdelbeginFL \DIFdelFL{2.0     }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-1.4 }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{- }& \DIFaddendFL 6 & 3.2 & \DIFdelbeginFL \DIFdelFL{-5.3          }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{9.5 }\DIFaddendFL & 1.5 & \DIFdelbeginFL \DIFdelFL{9.5            }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-5.3 }\DIFaddendFL \\
NLDAS & \DIFdelbeginFL \DIFdelFL{-1.4     }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{2.0 }\DIFaddendFL & 1.4 & \DIFdelbeginFL \DIFdelFL{2.0     }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-1.4 }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{- }& \DIFaddendFL 16 & 2.9 & \DIFdelbeginFL \DIFdelFL{-5.2          }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{6.3 }\DIFaddendFL & 2.1 & \DIFdelbeginFL \DIFdelFL{6.3            }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-5.2 }\DIFaddendFL \\
JRA & \DIFdelbeginFL \DIFdelFL{-1.4     }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{2.0 }\DIFaddendFL & 1.4 & \DIFdelbeginFL \DIFdelFL{2.0     }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-1.4 }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{- }& \DIFaddendFL 48 & 3.5 & \DIFdelbeginFL \DIFdelFL{-4.4          }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{5.3 }\DIFaddendFL & 2.6 & \DIFdelbeginFL \DIFdelFL{5.3            }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-4.4 }\DIFaddendFL \\
E3SM & \DIFdelbeginFL \DIFdelFL{-1.4     }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{2.0 }\DIFaddendFL & 1.4 & \DIFdelbeginFL \DIFdelFL{2.0     }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-1.4 }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{- }& \DIFaddendFL 58 & 4.1 & \DIFdelbeginFL \DIFdelFL{-5.2          }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{6.5 }\DIFaddendFL & 2.4 & \DIFdelbeginFL \DIFdelFL{6.5            }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-5.2 }\DIFaddendFL \\ \hline
RELATIVE      &         &         &         &        &     &              &              &                \DIFaddbeginFL &                \DIFaddendFL \\ \hline
L15 & \DIFdelbeginFL \DIFdelFL{-1.5     }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{2.0 }\DIFaddendFL & 0.6 & \DIFdelbeginFL \DIFdelFL{2.0     }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-1.5 }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{- }& \DIFaddendFL 20 & 5.2 & \DIFdelbeginFL \DIFdelFL{-5.3          }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{6.2 }\DIFaddendFL & 1.1 & \DIFdelbeginFL \DIFdelFL{6.2            }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-5.3 }\DIFaddendFL \\
NLDAS & \DIFdelbeginFL \DIFdelFL{-0.8     }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{2.0 }\DIFaddendFL & 1.4 & \DIFdelbeginFL \DIFdelFL{2.0     }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-0.8 }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{- }& \DIFaddendFL 20 & 2.8 & \DIFdelbeginFL \DIFdelFL{-4.2          }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{7.2 }\DIFaddendFL & 2.1 & \DIFdelbeginFL \DIFdelFL{7.2            }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-4.2 }\DIFaddendFL \\
JRA & \DIFdelbeginFL \DIFdelFL{-1.9     }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{2.0 }\DIFaddendFL & 1.6 & \DIFdelbeginFL \DIFdelFL{2.0     }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-1.9 }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{- }& \DIFaddendFL 40 & \DIFdelbeginFL \DIFdelFL{3.2          }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{3.1 }\DIFaddendFL & \DIFdelbeginFL \DIFdelFL{-4.1          }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{5.1 }\DIFaddendFL & 2.5 & \DIFdelbeginFL \DIFdelFL{5.1            }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-4.1 }\DIFaddendFL \\
E3SM & \DIFdelbeginFL \DIFdelFL{-2.2     }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{2.0 }\DIFaddendFL & 1.8 & \DIFdelbeginFL \DIFdelFL{2.0     }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-2.2 }\DIFaddendFL & \DIFaddbeginFL \DIFaddFL{- }& \DIFaddendFL 41 & 4.1 & \DIFdelbeginFL \DIFdelFL{-8.0          }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{6.9 }\DIFaddendFL & 2.9 & \DIFdelbeginFL \DIFdelFL{6.9 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{-8.0 }\DIFaddendFL \\ \hline
\DIFaddbeginFL \DIFaddFL{RELATIVE\_F14      }&         &         &         &        &     &              &              &                &                \\ \DIFaddendFL \hline
\DIFaddbeginFL \DIFaddFL{L15 }& \DIFaddFL{2.0 }& \DIFaddFL{0.6 }& \DIFaddFL{-1.5 }& \DIFaddFL{20 }& \DIFaddFL{20 }& \DIFaddFL{4.9 }& \DIFaddFL{6.0 }& \DIFaddFL{1.0 }& \DIFaddFL{-5.3 }\\
\DIFaddFL{NLDAS }& \DIFaddFL{2.0 }& \DIFaddFL{1.4 }& \DIFaddFL{-0.8 }& \DIFaddFL{20 }& \DIFaddFL{13 }& \DIFaddFL{3.0 }& \DIFaddFL{4.4 }& \DIFaddFL{1.9 }& \DIFaddFL{-5.1 }\\
\DIFaddFL{JRA }& \DIFaddFL{2.0 }& \DIFaddFL{1.6 }& \DIFaddFL{-1.9 }& \DIFaddFL{20 }& \DIFaddFL{40 }& \DIFaddFL{3.0 }& \DIFaddFL{5.3 }& \DIFaddFL{2.4 }& \DIFaddFL{-4.2 }\\
\DIFaddFL{E3SM }& \DIFaddFL{2.0 }& \DIFaddFL{1.8 }& \DIFaddFL{-2.2 }& \DIFaddFL{20 }& \DIFaddFL{41 }& \DIFaddFL{4.0 }& \DIFaddFL{6.8 }& \DIFaddFL{2.8 }& \DIFaddFL{-7.8 }\\   \hline
...
The sign of dSWE is flipped here for ease of visualization with the other metrics (i.e., positive dSWE in both figures denotes \DIFdelbegin \DIFdel{snowpack}\DIFdelend \DIFaddbegin \DIFadd{snow ablation}\DIFaddend ).
...
The probability distribution functions of dSWE highlight an even more complex picture, with both NLDAS and L15 having narrower distributions with smaller magnitudes \DIFdelbegin \DIFdel{.
Of note, }\DIFdelend \DIFaddbegin \DIFadd{than E3SM.
}\DIFaddend JRA and E3SM have broader distributions (i.e., longer tails) but the JRA distribution is more skewed, with frequent low dSWE events whereas E3SM has a more uniform distribution of dSWE rates.
...
\DIFaddbegin \label{subsec:singleyeareval}
\DIFaddend 

\begin{figure}
\begin{tabular}{cc}
\begin{overpic}[width=0.45\linewidth]{{figs/cropped/L15_1995_events}.pdf}
\put (\DIFdelbeginFL \DIFdelFL{11.5}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{10.5}\DIFaddendFL ,40) {\contour{white}{\large a.}}
\end{overpic}
&
\begin{overpic}[width=0.45\linewidth]{{figs/cropped/NLDAS_1995_events}.pdf}
\put (\DIFdelbeginFL \DIFdelFL{11.5}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{10.5}\DIFaddendFL ,40) {\contour{white}{\large b.}}
\end{overpic}
\vspace{0.10cm} \\
\begin{overpic}[width=0.45\linewidth]{{figs/cropped/JRA_1995_events}.pdf}
\put (\DIFdelbeginFL \DIFdelFL{11.5}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{10.5}\DIFaddendFL ,40) {\contour{white}{\large c.}}
\end{overpic}
&
\begin{overpic}[width=0.45\linewidth]{{figs/cropped/E3SM_1995_events}.pdf}
\put (\DIFdelbeginFL \DIFdelFL{11.5}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{10.5}\DIFaddendFL ,40) {\contour{white}{\large d.}}
\end{overpic}
\end{tabular}
\caption{Time series of daily precipitation (PRECIP; green), surface runoff (ROF; orange), and SWE loss (dSWE; pink) during \DIFdelbeginFL \DIFdelFL{WY95 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{WY96 }\DIFaddendFL for each of the four products. Vertical purple shading denotes periods flagged as an RoS event. At the top of each time series are colored bars that denote extreme values (greater than the \DIFdelbeginFL \DIFdelFL{75th }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{90\% }\DIFaddendFL percentile) of observed USGS streamflow at Harrisburg, Pennsylvania \DIFaddbeginFL \DIFaddFL{(\#01570500)}\DIFaddendFL , with \DIFdelbeginFL \DIFdelFL{very dark hues }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{black }\DIFaddendFL representing \DIFdelbeginFL \DIFdelFL{99th }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{99.9th }\DIFaddendFL percentile values over the 1980-2005 period.}
...
\put (\DIFdelbeginFL \DIFdelFL{12}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{13}\DIFaddendFL ,63) {\contour{white}{\Large a.}}
\end{overpic}
&
\begin{overpic}[width=0.45\linewidth]{{figs/cropped/NLDAS_1995_scatplot}.pdf}
\put (\DIFdelbeginFL \DIFdelFL{12}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{13}\DIFaddendFL ,63) {\contour{white}{\Large b.}}
\end{overpic}
\vspace{0.10cm} \\
\begin{overpic}[width=0.45\linewidth]{{figs/cropped/JRA_1995_scatplot}.pdf}
\put (\DIFdelbeginFL \DIFdelFL{12}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{13}\DIFaddendFL ,63) {\contour{white}{\Large c.}}
\end{overpic}
&
\begin{overpic}[width=0.45\linewidth]{{figs/cropped/E3SM_1995_scatplot}.pdf}
\put (\DIFdelbeginFL \DIFdelFL{12}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{13}\DIFaddendFL ,63) {\contour{white}{\Large d.}}
\end{overpic}

\end{tabular}
\caption{Bubble plots comparing precipitation (PRECIP; y-axis), SWE change (dSWE; x-axis), and surface runoff (size of bubble) for each of the four datasets during \DIFdelbeginFL \DIFdelFL{WY95}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{WY96}\DIFaddendFL . For each bubble, the values come from the same calendar day. Filled bubbles were flagged by the algorithm as an RoS event (purple shaded periods in Fig. \ref{fig:yr-timeseries-comp}).}
...
Figure \ref{fig:yr-timeseries-comp} shows the SRB daily \DIFdelbegin \DIFdel{WY95 }\DIFdelend \DIFaddbegin \DIFadd{WY96 }\DIFaddend time series of basin-integrated PRECIP, ROF, and dSWE.
...
At the top of each panel is a stripe with four different shadings, representing days where streamflow exceeded the \DIFdelbegin \DIFdel{75th, }\DIFdelend 90th, 95th, \DIFdelbegin \DIFdel{and }\DIFdelend 99th\DIFaddbegin \DIFadd{, and 99.9th }\DIFaddend relative percentiles over the 1985-2005 period at the US Geological Survey (USGS) gage at Harrisburg, Pennsylvania (USGS \#01570500), with deepest blues \DIFaddbegin \DIFadd{and black }\DIFaddend representing highly anomalous flow conditions.

The January 17-18, 1996 event is readily apparent for three of the four data products (NLDAS, JRA, and E3SM).
Observed streamflow spiked during and immediately after the RoS event as shown by the dark blue stripes at the top of each time series, indicating a lag between when the RoS event algorithm identifies changes in PRECIP, dSWE, and ROF and the observed daily streamflow gage observation at Harrisburg (namely, the \DIFdelbegin \DIFdel{99th }\DIFdelend \DIFaddbegin \DIFadd{99.9th }\DIFaddend percentile streamflow exceedance).
...
All products also indicate a second, more moderate RoS event during the last third of February, although L15 and E3SM (JRA\DIFdelbegin \DIFdel{and E3SM}\DIFdelend ) show larger dSWE (ROF) signals than the other \DIFdelbegin \DIFdel{two }\DIFdelend datasets.
...
Another way to visualize the \DIFdelbegin \DIFdel{WY95 }\DIFdelend \DIFaddbegin \DIFadd{WY96 }\DIFaddend RoS events is shown in Fig. \ref{fig:bubble-comp}.
...
\DIFaddbegin \label{subsec:evaluationsingleevent}
\DIFaddend 

%\begin{figure}
%\noindent\includegraphics[width=0.95\textwidth]{figs/cropped/merged_SWE_1995.png}
%DIF < \caption{Daily SWE timeseries averaged over the SRB during WY95 for all four data products. Shaded in blue are the same events from Fig. \ref{fig:yr-timeseries-comp}.}
%DIF > \caption{Daily SWE timeseries averaged over the SRB during WY96 for all four data products. Shaded in blue are the same events from Fig. \ref{fig:yr-timeseries-comp}.}
...
\put (\DIFdelbeginFL \DIFdelFL{11}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{12}\DIFaddendFL ,39.5) {\contour{white}{\large a.}}
\end{overpic}
&
\begin{overpic}[width=0.45\linewidth]{{figs/cropped/NLDAS_1995_SWE}.pdf}
\put (\DIFdelbeginFL \DIFdelFL{11}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{12}\DIFaddendFL ,39.5) {\contour{white}{\large b.}}
\end{overpic}
\vspace{0.10cm} \\
\begin{overpic}[width=0.45\linewidth]{{figs/cropped/JRA_1995_SWE}.pdf}
\put (\DIFdelbeginFL \DIFdelFL{11}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{12}\DIFaddendFL ,39.5) {\contour{white}{\large c.}}
\end{overpic}
&
\begin{overpic}[width=0.45\linewidth]{{figs/cropped/E3SM_1995_SWE}.pdf}
\put (\DIFdelbeginFL \DIFdelFL{11}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{12}\DIFaddendFL ,39.5) {\contour{white}{\large d.}}
\end{overpic}
\end{tabular}
\caption{Daily SWE timeseries averaged over the SRB during \DIFdelbeginFL \DIFdelFL{WY95 }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{WY96 }\DIFaddendFL for all four data products. Shaded in blue are the same events from Fig. \ref{fig:yr-timeseries-comp}.}
\label{fig:allswewy95}
\end{figure}
...
Fig. \ref{fig:allswewy95} shows the SRB evolution of total on-surface SWE over \DIFdelbegin \DIFdel{WY95 }\DIFdelend \DIFaddbegin \DIFadd{WY96 }\DIFaddend for all datasets.
...
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \begin{overpic}[width=0.8\textwidth]{figs/cropped/precip_vs_t_1996.pdf}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \begin{overpic}[width=0.8\textwidth]{figs/cropped/precip_vs_t.pdf}
\DIFaddendFL \put (7,65) {\contour{white}{\Large a.}}
...
When combined, this explains the lack of a decline in L15 SWE and subsequent \DIFdelbegin \DIFdel{large }\DIFdelend \DIFaddbegin \DIFadd{lack of a }\DIFaddend spike in ROF in Fig. \ref{fig:yr-timeseries-comp}.
...
\DIFaddbegin \subsection{\DIFadd{Generalizability to other basins}}
\label{subsec:other-basins}

\DIFadd{Finally, while we focus on the SRB in this manuscript, it is beneficial to evaluate whether the algorithm can detect other well-known historical RoS events in the US and whether the dataset-to-dataset variability observed in the SRB occurs elsewhere.
...
These conditions led to significant snowmelt, exacerbating the flooding \mbox{%DIFAUXCMD
\citep{halpert1997climate,colle2000february}}\hskip0pt%DIFAUXCMD
.
The 1997 New Year's flood event in California was the most financially devastating flood in the state's history with damages totaling \$1.6 USD billion, ranked as the second most severe superflood between 1950 and 2010 across the western United States \mbox{%DIFAUXCMD
\citep{tarouilly2021western}}\hskip0pt%DIFAUXCMD
.
Over half a million individuals were displaced, and 43 out of 58 California counties were declared disaster zones \mbox{%DIFAUXCMD
\citep{lott1997}}\hskip0pt%DIFAUXCMD
.
Preceding storms in late November and December contributed to elevated soil moisture and a substantial snowpack, setting the stage for extreme flooding exacerbated by heavy precipitation and an intense, warm, atmospheric river event on New Year's Day of 1997 \mbox{%DIFAUXCMD
\citep{galewsky2005moist,rhoades2023recreating}}\hskip0pt%DIFAUXCMD
.
}

\begin{figure}
\noindent\includegraphics[width=0.7\textwidth]{figs/cropped/other_basins_ROF.pdf}
\caption{\DIFaddFL{Basin shapefile domains for (a.) the Willamette River Basin (WRB) and (b.) the Sacramento River Basin (SacRB). Contoured is the surface runoff field from JRA on (a.) February 7th, 1996, and (b.) December 30th, 1996.}}
\label{fig:otherbasins}
\end{figure}

\DIFadd{To probe these events, we first acquire shapefiles defining both the WRB and SacRB, shown in Fig. \ref{fig:otherbasins}.
...
\DIFadd{Fig. \ref{fig:ros-wrb} shows the WY96 results over the WRB. The February flood event is clearly evident in the streamflow shading along the top of the panels on the left (black colors indicating 99.9\% streamflow).
...
\put \DIFaddFL{(11,39.5) }{\contour{white}{\large a.}}
\end{overpic}
&
\begin{overpic}[width=\linewidth]{figs/cropped/L15_WillametteBasin_scatplot.pdf}
\put \DIFaddFL{(14,61.0) }{\contour{white}{\large b.}}
\end{overpic}
\\
\begin{overpic}[width=\linewidth]{figs/cropped/NLDAS_WillametteBasin_events.pdf}
\put \DIFaddFL{(11,39.5) }{\contour{white}{\large c.}}
\end{overpic}
&
\begin{overpic}[width=\linewidth]{figs/cropped/NLDAS_WillametteBasin_scatplot.pdf}
\put \DIFaddFL{(14,61.0) }{\contour{white}{\large d.}}
\end{overpic}
\\
\begin{overpic}[width=\linewidth]{figs/cropped/JRA_WillametteBasin_events.pdf}
\put \DIFaddFL{(11,39.5) }{\contour{white}{\large e.}}
\end{overpic}
&
\begin{overpic}[width=\linewidth]{figs/cropped/JRA_WillametteBasin_scatplot.pdf}
\put \DIFaddFL{(14,61.0) }{\contour{white}{\large f.}}
\end{overpic}
\\
\begin{overpic}[width=\linewidth]{figs/cropped/E3SM_WillametteBasin_events.pdf}
\put \DIFaddFL{(11,39.5) }{\contour{white}{\large g.}}
\end{overpic}
&
\begin{overpic}[width=\linewidth]{figs/cropped/E3SM_WillametteBasin_scatplot.pdf}
\put \DIFaddFL{(14,61.0) }{\contour{white}{\large h.}}
\end{overpic}
\\
\end{tabular}
\caption{\DIFaddFL{As in Figs. \ref{fig:yr-timeseries-comp} (a., c., e., g.) and \ref{fig:bubble-comp} (b., d., f., h.), except for the Willamette River Basin (WRB) during WY96. The striped gauge percentiles along the top of the left panels are derived from daily streamflow from USGS gage \#14211720 (Willamette River at Portland, Oregon).}}
\label{fig:ros-wrb}
\end{figure}

\DIFadd{Figure \ref{fig:ros-sacrb} shows the WY97 results over the SacRB.
...
\\
\end{tabular}
\caption{\DIFaddFL{As in Figs. \ref{fig:yr-timeseries-comp} (a., c., e., g.) and \ref{fig:bubble-comp} (b., d., f., h.), except for the Sacramento River Basin (SacRB) during WY97. The striped gauge percentiles along the top of the left panels are derived from daily streamflow from USGS gage \#14211720 (Sacramento River at Verona, California).}}
\label{fig:ros-sacrb}
\end{figure}

\DIFadd{While this should only be considered a cursory investigation, it provides additional data points that the methodology can be applicable to other basins susceptible to RoS flooding.
...
\DIFaddbegin \DIFadd{We include surface runoff as a criterion to incorporate information regarding the land surface model soil conditions before and during RoS events.
...
%DIFDELCMD < %%%
\DIFdelend Normalizing these thresholds by each dataset's climatology (relative thresholds based on daily percentiles) improves agreement.
